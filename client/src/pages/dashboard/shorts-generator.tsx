...
`;
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/generate-video', {
        ...
        body: JSON.stringify({
          ...
          font_size: 75 // default font size
        })
      });
      ...
      const interval = window.setInterval(checkVideoStatus, 5000);
  ...
  const fetchAndDisplayVideo = async (taskId: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/download/${taskId}`, {
        headers: {
  ...
  const checkVideoStatus = async () => {
    if (!videoTask?.task_id) return;

    try {
      const response = await fetch(`http://localhost:8000/task/${videoTask.task_id}`);
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
      ...
      // Update video task with all the data
      setVideoTask({
        task_id: taskData.task_id,
        status: taskData.status,
      ...
      // If task is completed, fetch the video
      if (taskData.status === 'completed' && taskData.output_file && !videoBlob) {
        console.log('Video ready, fetching...', taskData.output_file);
        await fetchAndDisplayVideo(taskData.task_id);
      }
  ...
  const uploadToYoutube = async (taskId: string) => {
    if (!videoTask || !generatedScript) return;
    
    setUploadingToYoutube(true);
    try {
      const response = await fetch(`http://localhost:8000/upload-youtube/${taskId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          title: generatedScript.topic,
          description: `${generatedScript.script}\n\nGenerated by CryptoPilot AI`,
          tags: ['crypto', 'cryptocurrency', 'news', 'shorts']
        })
      });
                ...
                {videoBlob ? (
                  <>
                    ...
                    <div className="mt-4 flex justify-end space-x-2">
                      ...
                      <Button
                        onClick={() => {
                          const url = `http://localhost:8000/download/${videoTask.task_id}`;
                      ...
                      {!videoTask.youtube_url && (
                        <Button
                          onClick={() => uploadToYoutube(videoTask.task_id)}
                      ...
                      {videoTask.youtube_url && (
                        <Button
                          onClick={() => window.open(videoTask.youtube_url, '_blank')}
                          variant="default"
                        >
                          <Youtube className="mr-2 h-4 w-4" />
                          View on YouTube
