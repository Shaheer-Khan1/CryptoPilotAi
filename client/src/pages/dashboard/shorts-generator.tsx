...
`;
  ...
  const generateVideo = async () => {
    ...
    try {
      ...
      const interval = window.setInterval(() => checkWebhookStatus(taskData.task_id), 3000);
  ...
  // Function to check webhook status (checks if video was received via webhook)
  const checkWebhookStatus = async (taskId: string) => {
    try {
      const response = await fetch(`/api/video-status/${taskId}`);
      
      if (response.ok) {
        // Video received via webhook!
        const videoData = await response.json();
        console.log('Video received via webhook:', videoData);
        
        setVideoTask({
          task_id: videoData.task_id,
          status: videoData.status,
          duration: videoData.duration,
          output_file: `video_${taskId}.mp4`
        });

        // Load the video
        if (!videoBlob) {
          const videoUrl = `/api/video/${taskId}`;
        ...
        console.error('Error checking webhook status:', response.status);
      }
  ...
  // Function to check video status (fallback for non-webhook)
